            ...
            <>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-display text-sm mt-1">{item.price.currencyCode} {parseFloat(item.price.amount).toFixed(2)}</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t bg-background">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-display text-xl">{items[0]?.price.currencyCode || '₹'} {totalPrice.toFixed(2)}</span>
                </div>
                <Button onClick={handleCheckout} className="w-full bg-gradient-saffron text-primary-foreground hover:opacity-90" size="lg" disabled={items.length === 0 || isLoading || isSyncing}>
